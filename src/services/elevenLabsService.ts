...
export async function textToSpeech(text: string): Promise<ArrayBuffer> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const errorMessage = errorData.detail?.message || errorData.message || `API error: ${response.status}`;

      if (response.status === 401) {
        throw new Error('Invalid API key. Please check your ElevenLabs API key in the .env file. Keys should start with "sk_" and be around 32+ characters.');
      }
